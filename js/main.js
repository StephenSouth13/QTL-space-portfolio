...
async function loadProfileData() {
    try {
        ...
        if (skillsContainer) {
            ...
            skillsContainer.innerHTML = `
                <div class="skill-category">
                    <h3>Frontend</h3>
                    ${frontend.map((skill) => `<div class="skill"><p>${skill}</p><div class="skill-bar"><div class="skill-level" style="width: ${Math.random() * 40 + 60}%;"></div></div></div>`).join('')}
                </div>
                <div class="skill-category">
                    <h3>Backend</h3>
                    ${backend.map((skill) => `<div class="skill"><p>${skill}</p><div class="skill-bar"><div class="skill-level" style="width: ${Math.random() * 40 + 60}%;"></div></div></div>`).join('')}
                </div>
                <div class="skill-category">
                    <h3>Version Control</h3>
                    ${versionControl.map((skill) => `<div class="skill"><p>${skill}</p><div class="skill-bar"><div class="skill-level" style="width: ${Math.random() * 40 + 60}%;"></div></div></div>`).join('')}
                </div>
                <div class="skill-category">
                    <h3>Tools</h3>
                    ${tools.map((skill) => `<div class="skill"><p>${skill}</p><div class="skill-bar"><div class="skill-level" style="width: ${Math.random() * 40 + 60}%;"></div></div></div>`).join('')}
                </div>
                <div class="skill-category">
                    <h3>Game Development</h3>
                    ${gameDev.map((skill) => `<div class="skill"><p>${skill}</p><div class="skill-bar"><div class="skill-level" style="width: ${Math.random() * 40 + 60}%;"></div></div></div>`).join('')}
                </div>
            `;
        }
        // Experience
        ...
        if (portfolioGrid) {
            portfolioGrid.innerHTML = profile.projects.map((project) => `
        ...
        if (achievementsContainer) {
            achievementsContainer.innerHTML = `
                <div class="achievement-category">
                    <h3>Academic</h3>
                    <ul>${profile.achievements.academic.map((ach) => `<li>${ach}</li>`).join('')}</ul>
                </div>
                <div class="achievement-category">
                    <h3>Sports</h3>
                    <ul>${profile.achievements.sports.map((ach) => `<li>${ach}</li>`).join('')}</ul>
                </div>
                <div class="achievement-category">
                    <h3>Pacing</h3>
                    <ul>${profile.achievements.pacing.map((ach) => `<li>${ach}</li>`).join('')}</ul>
                </div>
                <div class="achievement-category">
                    <h3>Community</h3>
                    <ul>${profile.achievements.community.map((ach) => `<li>${ach}</li>`).join('')}</ul>
                </div>
            `;
        }
        // Contact
